...
- Herblore provides selectable Saradomin brew, Super restore, and 1-tick extended super antifire routes. Shared potion economics apply Prescription goggles and charged Alchemist's amulet effects only where eligible, price Amulets of chemistry used for charges, and decant all finished potion output to four-dose items for sale.
...
XP goals, start overrides, selected training methods, personal rates, and money-making skill allocations are stored per RSN at:
...
Each skill owns a folder under `src\RunescapeTools.Infrastructure\Training\Skills`. Its catalogue composes independently maintained method definitions, while a skill-local `Global.cs` is added only when multiple methods genuinely share rules or assumptions. `MainEhpCatalogue` only composes skill definitions in canonical Hiscores order, and verified item IDs remain centralized. Every registered route appears in that skill's dropdown, and all route-specific market items are included when the planner refreshes prices.
